...
class WebService {
  Future<Guard> fetchGuard(String id, String number) async {
    var url = Uri.parse('https://31c483aa6e19.ngrok.io/app_connection/$id/');
  ...
  Future<Route> fetchRoute(String id) async {
    var url =
        Uri.parse('https://31c483aa6e19.ngrok.io/app_connection/Route/$id/');
  ...
  Future<Station> fetchStation(String id) async {
    var url =
        Uri.parse('https://31c483aa6e19.ngrok.io/app_connection/Station/$id/');
    var response = await http.get(url);
    if (response.statusCode == 200) {
      var result = response.body.replaceAll('\\', '');
